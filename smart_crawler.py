...
load_dotenv(dotenv_path='/mnt/e/LexaWebapp/.env')
...
def gemini_extract_image_data(image_url):
    ...
    try:
        ...
        # 1) Summarize
        resp_summary = client.generate_content([
        ...
        for link in top_links:
            ...
            for sub_url in found_urls:
                ...
                if mime_type_short == 'pdf':
                    ...
                    pdf_content = gemini_extract_pdf_content(sub_url, objective)
                    if pdf_content:
                        pdf_image_append += f"\n\n---\n[PDF from {sub_url}]:\n{pdf_content}"
...
def main():
    url = "https://docs.cloudbase.net/run/"
    objective = "学习如何使用和部署 Cloudbase 云托管，包括快速入门、部署流程、服务管理和不同语言（如Node.js, Python）的最佳实践。"
    
    print(f"{Colors.BLUE}Website to crawl: {url}{Colors.RESET}")
