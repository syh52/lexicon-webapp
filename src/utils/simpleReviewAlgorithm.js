...
export class SimpleReviewScheduler {
  constructor(config = DAILY_CONFIGS.standard) {
    this.config = config;
   ...
   * 获取今日学习队列
   */
  getDailyStudyQueue(allWords, userWords = []) {
    const now = new Date();
   ...
   * 转换为SimpleWordRecord实例
   */
  convertToWordRecords(allWords, userWords) {
   ...
   * 分类单词
   */
  categorizeWords(wordRecords) {
    const now = new Date();
    const newWords = [];
    const dueReviewWords = [];
    const futureReviewWords = [];
    ...
    // 按优先级排序
    dueReviewWords.sort((a, b) => a.nextReview - b.nextReview); // 过期时间越长优先级越高
    futureReviewWords.sort((a, b) => a.nextReview - b.nextReview); // 即将到期的优先
 ...
 * 便捷函数：获取今日学习队列
 */
export function getDailyStudyQueue(allWords, userWords, config = DAILY_CONFIGS.standard) {
 ...
 * 便捷函数：获取学习统计
 */
export function getStudyStats(wordRecords) {
 ...
 * 便捷函数：处理用户选择
 */
export function processUserChoice(wordRecord, isKnown) {
  if (isKnown) {
